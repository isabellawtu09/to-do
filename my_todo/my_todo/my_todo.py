        ...
        try:
            with rx.session() as session:
                ...
                if user and user.password == password:
                    ...
                    # Set role to Assignee by default when logging in
                    user.role = "Assignee"
                    session.add(user)
                    session.commit()
                    self.current_user = user
...
def role_switcher():
    return rx.cond(
        ...
        placeholder="Switch Role",
            ...
            on_change=lambda role: State.set_role(role),
        ),
...
def dashboard():
    return rx.vstack(
        ...
        rx.cond(
            (State.current_user != None) & (State.current_user.role == "Manager"),
            add_item_form(),
            rx.text("You do not have permission to assign or delete tasks.", color="red"),
        ),
        rx.table.root(
            rx.table.header(
                rx.table.row(
